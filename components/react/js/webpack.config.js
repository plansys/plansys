...
if (production) {
    ...
    rimraf(uidir, function() {
        copy(dlldir + path.sep + 'dll.*', uidir, function() {});
        copy(__dirname + path.sep + 'babel.min.js', uidir, function() {});
    });
}
else {
...
if (production) {
    ...
    output.publicPath = 'plansys/components/react/ui/';
}
else {
...
listDir(uieldir, (res) => {
    return res.map((d) => {
        ...
        if (!!el) {
            ...
            if (fs.existsSync(d + path.sep + 'index.js')) {
                uielements.push(el);
...
plugins.push(new webpack.DefinePlugin({
    'CACHEHASH': JSON.stringify((new Date()).getTime()),
